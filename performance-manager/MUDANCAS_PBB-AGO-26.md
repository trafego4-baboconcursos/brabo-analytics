...
### 20. Auditoria de públicos — Quente Principal (montada manualmente) tinha 6 de 7 grupos com público errado
- **Contexto:** usuário pediu pra revisar TODAS as campanhas já existentes na Criadora de Públicos (não só as migradas por mim) e conferir se os públicos batem com a conta do Felipe Graton. Explicitamente **não mexer** na campanha "Quente Potencial" (arquivada intencionalmente, fora de escopo).
- **Método:** comparado grupo a grupo (por nome normalizado) os públicos de inclusão/exclusão de cada ad set nas 4 campanhas já existentes (Quente Principal manual + as 3 "Teste" migradas por mim) contra a origem.
- **Resultado:** Quente Teste, Frio Teste e Específico Teste — **100% corretas**, nenhum ajuste necessário (migração anterior já tinha ficado certa). **Quente Principal (montada manualmente pelo usuário) tinha 6 dos 7 grupos com público incorreto** — padrão de erro consistente: público genérico `[SITE] - Visitou o site` no lugar de `[SITE] Visitou Brabo Concursos`, `[SITE] Cadastrados [FEV/JUN-26]` no lugar de `[Lista] Cadastrados [FEV/JUN-26]`, e referências erradas ao lançamento **PBB-JUN-26** (antigo) em vez de PBB-AGO-26 em duas listas de "Viu Captação". Faltava também a exclusão de `[Lista] Cadastrados [PBB-ABR-26]` em 2 grupos.
...
### 23. Fechamento da migração: Reels completos, Quente Potencial criada, duplicatas resolvidas, orçamento consolidado
- **Contexto:** confirmado que as 12 campanhas antigas na conta Felipe Graton (`act_438212624024216`) estão todas **PAUSED** — migração para a Criadora de Públicos 100% consolidada, sem mais divisão 60/40 entre contas.
- **Reels:** confirmado que Frio Reels e Específico Reels já estavam com os 25 anúncios corretos (feito em paralelo pelo usuário/automação anterior). **Quente Reels** grupo "00 - Cadastrados Antigos" tinha 16 anúncios (6 duplicatas zeradas criadas por engano em 08/08, redundantes com versões que o usuário já tinha ativado manualmente em 07/08) — **6 duplicatas deletadas**, voltou a 10 anúncios. As 3 campanhas Reels foram **ativadas**.
- **Quente Potencial:** não existia na Criadora. Criada do zero (`120249481830270549`) copiando a targeting exata dos 7 grupos (00-06) da Quente Principal já corrigida + trazidos os 7 anúncios de referência (AD050/108/123/143/149/150/154) da conta antiga para cada grupo — 49/49 criados e ativados.
- **Frio Potencial ↔ Frio Principal — troca de identidade:** usuário identificou que havia duas campanhas "Frio Principal" com nome idêntico (uma ativa com conteúdo correto de Principal, outra pausada e **quebrada** — 2 de 3 grupos sem público nenhum). Usuário renomeou a ativa (com conteúdo de Principal) para "Frio Potencial", pretendendo reaproveitar o slot. Corrigido: deletados os 24 anúncios de Principal que estavam nela por engano, criados os 21 anúncios corretos de Potencial (AD050/108/123/143/149/150/154 × 3 grupos). **Depois o usuário apagou essa campanha por engano** (tinha duas Frio Potencial); a que sobrou (`120246131764310549`, a original com 14 anúncios e histórico real de spend) foi auditada e tinha 2 códigos (AD124, AD148) fora do conjunto de referência — **pausados 6 anúncios** (2 códigos × 3 grupos), mantendo só os 7 anúncios corretos ativos por grupo.
- **Frio Principal restaurada:** a campanha pausada e quebrada (`120246006626710549`) na verdade tinha **histórico real de spend** (mais de R$6 mil investidos desde 31/05) — não era uma duplicata órfã, era a campanha original. Corrigido o `targeting` dos 2 grupos sem público (Bancário, Interessados BB) usando as mesmas audiências/interesses da Frio Principal da conta antiga (todas já existiam na Criadora). **Ativada.**
- **Quente Principal [OLD]:** achado um grupo com rótulo `PBB-JUN-26` no nome (só o nome — o público já apontava certo pra PBB-AGO-26); renomeado pra `PBB-AGO-26`. Usuário decidiu **manter essa campanha ativa** (roda em paralelo com a Quente Principal nova) e contar o orçamento dela separadamente.
- **Consolidação do orçamento diário (`orcamento_diario.json`):** reestruturado de 14 chaves (metade apontando pra conta antiga, já pausada) para **16 chaves**, uma por campanha Meta ativa na Criadora (Principal/Potencial/Teste/Reels/New-Ads × Quente/Frio/Específico + `quente_principal_old`). Proporções por segmento derivadas das proporções já existentes no JSON anterior:
  - Quente: New-Ads = 22,4% do total; Principal (novo) = 20,56%; Principal [OLD] = 31,71% (dividido proporcional ao peso de orçamento atual entre as duas); Potencial = 21,33%; Teste = 2%; Reels = 2%.
  - Frio/Específico: New-Ads = 21%; Principal = 49%; Potencial = 20%; Teste = 5%; Reels = 5% (mesma lógica do carve-out de 30% pro New-Ads já usada no Google Ads, aplicada dentro da fatia de Principal).
  - Totais diários por público (Facebook Quente/Frio/Específico) batendo exatamente com o plano mestre de divisão por plataforma (21%/6%/6% do total, ver planilha).
  - Google Ads não precisou de nenhuma mudança de ID — não foi afetado pela migração de conta do Meta; os 9 IDs (campanha + orçamento) continuavam válidos e com os valores de 08/08 esperando a automação retomar.
