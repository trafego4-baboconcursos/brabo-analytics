...
> - 🟡 **Região/cidade** — testado ao vivo contra as duas APIs: **Meta devolve estado** via `breakdowns=region` (ETL novo, tabela `meta_ads_region_daily`, primeira carga feita: 33.889 linhas); **Google NÃO expõe** estado/cidade em nenhuma view de relatório da API (`geographic_view` e `user_location_view` só devolvem o ID do país, sempre Brasil — testado e confirmado, não é falta de permissão). **Nenhuma das duas plataformas dá cidade**, só estado. Segunda limitação descoberta: quando o Meta Insights é quebrado por `region`, a API **não devolve o action_type "lead"** no array de actions (só ações de engajamento — like, comment, video_view etc.), confirmado em ~2.400 linhas reais de Captação sem um único "lead". Por isso a seção "Qualidade por Estado" do Debriefing ficou com **investimento (Meta) × compradores/receita/ROAS (vendas)** — sem leads/CPL/conversão por estado, que não são recuperáveis dessa forma pela API. PI-AGO-26: SP lidera com R$120k investidos e ROAS 6,9x; cobertura de estado nas vendas é de 100% (2.494/2.494 compradores).
>
> **Atualização 26/08/2026 (item 5 da lista de coletas novas):**
> - ✅ **Base unificada "Caminho do Comprador"** (pauta item 7) — novo módulo `frontend/db_readers/caminho_comprador.py` junta por e-mail/telefone: lead (ADxxx do utm_term + plataforma + data de cadastro) → grupo WhatsApp (Normal/VIP) → pesquisa (respondeu?) → canal de venda. Resumo de funil no Debriefing + export CSV pessoa-a-pessoa (`/api/caminho-comprador.csv`) pra já rodar IA em cima, como a pauta pedia ("primeiro precisamos da base"). PI-AGO-26 (2.494 compradores): 70,9% com anúncio identificado, 95,6% em algum grupo (2.206 VIP), 33,2% respondeu pesquisa, 3,1% via Comercial/IA. "Participou do sorteio/live" continua fora — sem fonte.
