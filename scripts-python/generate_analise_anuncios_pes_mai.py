...
def encontrar_csv_leads_abr():
    base = Path(r'analises/[PES-MAI-26]')
    candidatos = []
    for pasta in [base / 'Active Campaign', base / 'active-campaing', base / 'Active campaign']:
        if pasta.exists():
            candidatos.extend(pasta.glob('*.csv'))
    if not candidatos:
        candidatos.extend(f for f in base.rglob('*.csv') if 'pes-mai-26' in f.name.lower() or 'lead' in f.name.lower())
    if not candidatos:
        raise FileNotFoundError('Arquivo de leads PES-MAI-26 não encontrado')
...
df_leads['criativo_original'] = df_leads[utm_content_col].astype(str).str.strip()
if utm_term_col:
    mascara_vazia = df_leads['criativo_original'].isin(['', 'nan', 'None'])
    df_leads.loc[mascara_vazia, 'criativo_original'] = df_leads.loc[mascara_vazia, utm_term_col].astype(str).str.strip()
df_leads_criativo = df_leads[df_leads['criativo_original'].astype(str).str.strip().ne('')].copy()
...
_hm_raw = pd.read_csv(r'analises/[PES-MAI-26]/Vendas/pes-mai-26-hotmart.csv', sep=';', encoding='utf-8')
_hm_raw['email'] = _hm_raw['Email do(a) Comprador(a)'].astype(str).str.strip().str.lower()
_hm_raw = _hm_raw[_hm_raw['email'].str.contains('@', na=False)].copy()
_tipo_h = next((c for c in _hm_raw.columns if 'tipo' in c.lower() and 'cobran' in c.lower()), None)
if _tipo_h:
    _par_h = 'Quantidade total de parcelas'
    _cob_h = 'Quantidade de cobranças'
    _h_norm = _hm_raw[_hm_raw[_tipo_h].astype(str).str.strip() != 'Recuperador Inteligente'].copy()
    _h_norm['valor_num'] = pd.to_numeric(_h_norm['Faturamento líquido do(a) Produtor(a)'], errors='coerce').fillna(0)
    _h_ri = _hm_raw[
        (_hm_raw[_tipo_h].astype(str).str.strip() == 'Recuperador Inteligente') &
        (pd.to_numeric(_hm_raw[_cob_h], errors='coerce').fillna(0) == 1)
    ].copy()
    _h_ri[_par_h] = pd.to_numeric(_h_ri[_par_h], errors='coerce').fillna(1)
    _h_ri['valor_num'] = pd.to_numeric(_h_ri['Faturamento líquido do(a) Produtor(a)'], errors='coerce').fillna(0) * _h_ri[_par_h]
    df_hotmart = pd.concat([_h_norm, _h_ri], ignore_index=True)
else:
...
# TMB - apenas vendas válidas (Vigente/Efetivado)
try:
    df_tmb = pd.read_csv(r'analises/[PES-MAI-26]/Vendas/pes-mai-26-tmb.csv', sep=';', encoding='utf-8')
except UnicodeDecodeError:
    df_tmb = pd.read_csv(r'analises/[PES-MAI-26]/Vendas/pes-mai-26-tmb.csv', sep=';', encoding='latin-1')
col_situacao = next((c for c in df_tmb.columns if 'situa' in str(c).lower()), None)
if col_situacao:
    status_norm = df_tmb[col_situacao].astype(str).str.strip().str.lower()
...
col_ticket = next((c for c in df_tmb.columns if 'icket' in str(c).lower() and 'pedido' in str(c).lower()), 'Ticket do pedido')
df_tmb['valor_num'] = pd.to_numeric(df_tmb[col_ticket].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
...
output_file = r'analises/[PES-MAI-26]/ANALISE_ANUNCIOS_[PES-MAI-26].html'
with open(output_file, 'w', encoding='utf-8') as f:
    f.write(adaptar_html_campaign(html))
