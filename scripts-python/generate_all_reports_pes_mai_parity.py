...
def adaptar_html_campaign(html):
    ...
    replacements = {
        ...
        "Felipe Graton Banco do Brasil": PRODUCT_NAME,
        ...
        "MA-Campanhas-Completas-PBB-ABR-26.csv": "Campanhas-Completas-pes-mai-26.csv",
        "Performance da campanha-pbb-abr-26.csv": "Performance da campanha-pes-mai-26.csv",
        "Performance dos anúncios-pbb-abr-26.csv": "Performance dos anúncios-pes-mai-26.csv",
        "Públicos-alvo-pbb-abr-26.csv": "Públicos-alvo-pes-mai-26.csv",
        "hotmart-pbb-abr-26.csv": "pes-mai-26-hotmart.csv",
        "tmb-pbb-abr-26.csv": "pes-mai-26-tmb.csv",
        "ANALISE_GOOGLE_ANUNCIOS_[PBB-ABR-26].html": "ANALISE_GOOGLE_ADS_[PES-MAI-26].html",
    }
...
def detectar_coluna(columns, terms, prefer=None):
    prefer = prefer or []
    normalized = {col: normalizar_texto(col) for col in columns}
    matches = [col for col, norm in normalized.items() if all(term in norm for term in terms)]
    if not matches:
        return None
    for token in prefer:
        for col in matches:
            if token in normalized[col]:
                return col
    return matches[0]


def filtrar_campanha_pes(df, column_name):
    if df is None or column_name not in df.columns:
        return df
    return df[df[column_name].astype(str).str.contains(CAMPAIGN_CODE, case=False, na=False)].copy()
...
try:
    df_ga_campanhas = pd.read_csv(GOOGLE_ADS_PATH / "Performance da campanha-pes-mai-26.csv", encoding='utf-8', skiprows=2)
    df_ga_campanhas = filtrar_campanha_pes(df_ga_campanhas, "Campanha")
...
try:
    df_ga_ads = pd.read_csv(GOOGLE_ADS_PATH / "Performance dos anúncios-pes-mai-26.csv", encoding='utf-8', skiprows=2)
    df_ga_ads = filtrar_campanha_pes(df_ga_ads, "Campanha")
...
try:
    df_ga_audiences = pd.read_csv(GOOGLE_ADS_PATH / "Públicos-alvo-pes-mai-26.csv", encoding='utf-8', skiprows=2)
    df_ga_audiences = filtrar_campanha_pes(df_ga_audiences, "Campanha")
...
# Carregar Meta Ads
try:
    df_meta = pd.read_csv(META_ADS_PATH / "Campanhas-Completas-pes-mai-26.csv", encoding='utf-8')
    df_meta = filtrar_campanha_pes(df_meta, "Nome da campanha")
...
try:
    ...
    if not leads_candidates:
        leads_candidates.extend(
            ...
            if "lead" in f.name.lower() or "pes-mai-26" in f.name.lower() or "escrevente" in f.name.lower()
        )
...
# Carregar Vendas
try:
    _hm_raw = pd.read_csv(VENDAS_PATH / 'pes-mai-26-hotmart.csv', sep=';', encoding='utf-8')
    ...
    if _hm_tipo:
        ...
        _hm_norm['valor_num'] = pd.to_numeric(_hm_norm['Faturamento bruto (sem impostos)'], errors='coerce').fillna(0)
        _hm_ri = _hm_raw[
        ...
        _hm_ri['valor_num'] = pd.to_numeric(_hm_ri['Faturamento bruto (sem impostos)'], errors='coerce').fillna(0) * _hm_ri[_hm_par]
        df_hotmart = pd.concat([_hm_norm, _hm_ri], ignore_index=True)
    else:
        df_hotmart = _hm_raw.copy()
    ...
    print(f'✓ Vendas Hotmart carregadas ({len(df_hotmart)} vendas, R$ {df_hotmart["valor_num"].sum():,.2f})')
except Exception as e:
    df_hotmart = None
...
try:
    try:
        df_tmb = pd.read_csv(VENDAS_PATH / 'pes-mai-26-tmb.csv', sep=';', encoding='utf-8')
    except UnicodeDecodeError:
        df_tmb = pd.read_csv(VENDAS_PATH / 'pes-mai-26-tmb.csv', sep=';', encoding='latin-1')
    _col_situacao = detectar_coluna(df_tmb.columns, ['situa'], prefer=['situa'])
    if _col_situacao:
        status_norm = df_tmb[_col_situacao].astype(str).str.strip().str.lower()
        df_tmb = df_tmb[status_norm.isin(['vigente', 'efetivado'])]
    _col_email_tmb = detectar_coluna(df_tmb.columns, ['mail']) or 'E-mail do Cliente'
    _col_ticket_tmb = detectar_coluna(df_tmb.columns, ['ticket', 'pedido']) or 'Ticket do pedido'
    df_tmb['email'] = df_tmb[_col_email_tmb].astype(str).str.strip().str.lower()
    df_tmb['valor_num'] = pd.to_numeric(df_tmb[_col_ticket_tmb].astype(str).str.replace(',', '.'), errors='coerce').fillna(0)
...
with open(ANALISES_PATH / "INDEX_[PES-MAI-26].html", "w", encoding="utf-8") as f:
    f.write(adaptar_html_campaign(html_index))
print("✓ INDEX criado")
...
with open(ANALISES_PATH / "ANALISE_META_ADS_[PES-MAI-26].html", "w", encoding="utf-8") as f:
    f.write(adaptar_html_campaign(html_meta))
...
with open(ANALISES_PATH / "ANALISE_GOOGLE_ADS_[PES-MAI-26].html", "w", encoding="utf-8") as f:
    f.write(adaptar_html_campaign(html_google))
...
with open(ANALISES_PATH / "ANALISE_LEADS_CONFRONTO_[PES-MAI-26].html", "w", encoding="utf-8") as f:
    f.write(adaptar_html_campaign(html_leads))
...
try:
    os.chdir(BASE_PATH)
    runpy.run_path(str(BASE_PATH / "scripts-python" / "generate_analise_anuncios_pes_mai.py"), run_name="__main__")
finally:
...
with open(ANALISES_PATH / "ANALISE_META_AUDIENCES_[PES-MAI-26].html", "w", encoding="utf-8") as f:
    f.write(adaptar_html_campaign(html_meta_audiences))
...
with open(ANALISES_PATH / "ANALISE_GOOGLE_AUDIENCES_[PES-MAI-26].html", "w", encoding="utf-8") as f:
    f.write(adaptar_html_campaign(html_google_audiences))
...
try:
    os.chdir(BASE_PATH)
    runpy.run_path(
        str(BASE_PATH / "scripts-python" / "generate_analise_vendas_final.py"),
        run_name="__main__",
        init_globals={
            "BASE": BASE_PATH,
            ...
            "HOTMART_FILE": "pes-mai-26-hotmart.csv",
            "TMB_FILE": "pes-mai-26-tmb.csv",
            "OUTPUT_FILE": "ANALISE_VENDAS_[PES-MAI-26].html",
        },
    )
finally:
    os.chdir(_cwd_original)
...
<body style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="recommendation-box">
                <strong>1. Subir a tese de método, cronograma e acompanhamento em toda a comunicação</strong><br>
                O Typeform indica que o fit do produto está menos em “curso para concurso” e mais em “parar de estudar no improviso”. Criativos, LP e remarketing devem enfatizar direção prática, constância e rotina possível.
                <br><a href="ANALISE_TYPEFORM_[PES-MAI-26].html" style="color:#856404">→ Ver Análise Typeform</a>
            </div>
            <div class="recommendation-box">
                <strong>2. Separar mídia por maturidade do lead</strong><br>
                Criar trilhas específicas para “do zero” e “já tentei estudar e me perdi”. A segunda tende a reconhecer valor mais rápido, enquanto a primeira precisa de educação e prova de viabilidade.
                <br><a href="ANALISE_META_AUDIENCES_[PES-MAI-26].html" style="color:#856404">→ Ver Públicos Meta</a> |
                <a href="ANALISE_GOOGLE_AUDIENCES_[PES-MAI-26].html" style="color:#856404">→ Ver Públicos Google</a>
            </div>
            <div class="recommendation-box">
                <strong>3. Tratar o público 18–22 como aquecimento, não como principal frente de fechamento</strong><br>
                Esse grupo gera interesse, mas aparece proporcionalmente menos entre compradores. Vale nutrir com conteúdo, prova social e condição comercial antes de pressionar oferta direta.
                <br><a href="ANALISE_TYPEFORM_[PES-MAI-26].html" style="color:#856404">→ Ver Diferenças de Perfil</a>
            </div>
            <div class="recommendation-box">
                <strong>4. Resolver a objeção financeira no comercial sem reduzir valor percebido</strong><br>
                O bloqueio mais recorrente nas respostas de alunos é preço/caixa. Parcelamento, acesso vitalício e economia de tempo precisam aparecer de forma mais explícita na oferta e no follow-up.
                <br><a href="ANALISE_VENDAS_[PES-MAI-26].html" style="color:#856404">→ Ver Vendas</a>
            </div>
            <div class="success-box">
                <strong>5. Usar Ivan Neto como garantia de segurança metodológica</strong><br>
                A autoridade do produto deve ser explorada como prova de clareza prática e caminho validado, e não apenas como branding. A promessa que mais fecha é “com o Ivan Neto eu sei como começar e como manter a rotina”.
            </div>
            <div class="success-box">
...
with open(ANALISES_PATH / "INSIGHTS_RECOMENDACOES_[PES-MAI-26].html", "w", encoding="utf-8") as f:
    f.write(adaptar_html_campaign(html_insights))
print("✓ Insights e Recomendações criada")


def aplicar_brabo_nav_em_html(html_path):
    try:
        html = html_path.read_text(encoding="utf-8")
    except Exception as exc:
        print(f"⚠️  Não foi possível ler {html_path.name}: {exc}")
        return False

    if "BRABO-NAV" in html:
        return False
    if "</body>" not in html:
        return False
    ...
    wrapped = re.sub(
        r"<body[^>]*>",
        lambda match: match.group(0) + "\n" + nav_html(active_campaign=CAMPAIGN_CODE, active_page_file=html_path.name, depth=1),
        html,
        count=1,
        flags=re.IGNORECASE,
    )
    ...
    if wrapped != html:
        ...
        print(f"✓ Frame Brabo aplicado em {html_path.name}")
        return True
    return False


for html_path in sorted(ANALISES_PATH.glob("*.html")):
    aplicar_brabo_nav_em_html(html_path)
